#!/bin/bash
curl -XGET -u username:password 'elasticsearch.domain.com/replace_with_indexname/replace_with_type/_search' -H 'Content-Type: application/json' -d '{"query": { "range" : { "indexdate" : { "gte" : "now-15m", "lt" : "now"}}}}' | python -c "import sys, json; print json.load(sys.stdin)['hits']['total']"
